<div align="center">
  <img src="docs/banner.png" width="100%" alt="EasyMerger">
</div>
# EasyMerger
...
## 🐧QQ交流群：335743211
## 😣解决的痛点：
- 1.引入合并前的智能判断：市面上的视频合并拼接软件比如（shanaencoder），几乎都没有无损合并（-c copy）前的判断，让用户自己判断。一批视频比如短剧剧集看似参数一致，但其实不一致，使用无损合并导致输出视频花屏、卡声音等。本程序引入合并前智能判断功能，不用人工判断（人工也判断不准），参数相同走无损合并通道（耗时超短），参数不同转码合并通道。
...
- 4.软件纯绿色：不会主动在除软件本身目录和工作目录外生成任何配置/缓存/中间文件，删除软件文件夹即可完成无残留卸载。
